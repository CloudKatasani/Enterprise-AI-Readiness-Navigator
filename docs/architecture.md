...
|---|---|---|
| Portal | `frontend/` | Portfolio index, then a page per assessed estate at `/estates/{snapshot_id}` carrying the executive, architect and steward views; plus the connector permission pack, the API documentation and data model views, and the methodology view |
...
| API documentation | `backend/eairn/integration.py`, `seed/integration_v1.yaml`, `frontend/app/api-docs/` | Deployment wiring per connector — auth modes, configuration keys, egress hosts, freshness and limits — merged with the registry's own capabilities, grants and call catalogs. Every catalogued statement and endpoint is re-run through the read-only guard as the view renders, and check coverage is derived from the check library's capability requirements |
| Data model view | `backend/eairn/datamodel.py`, `frontend/app/data-model/` | Every table introspected from `Base.metadata` at request time: columns typed as Postgres will create them, keys, indexes, foreign-key graph, live row counts, and the `CREATE TABLE` DDL a hyperscaler deployment gets. Tables are grouped by lifecycle family, and canonical tables carry the connector capability that populates them |
